...
st.write("Axelino Natanael Ndahawali")
...
with col1:
    st.subheader("Perbedaan Pola Pengunaan Layanan Bike Sharing antara Hari Kerja dan Hari Libur")
    workingday_data = data.groupby(by=["workingday"],as_index=False)[["cnt"]].sum()
    st.bar_chart(workingday_data,x="workingday",y=["cnt"])
with col2:
    st.subheader("Pengaruh Kondisi Cuaca terhadap Jumlah Pengguna Bike Sharing")
...
with col3:
    st.subheader("Perbedaan Pola Penggunaan Layanan Bike Sharing antara Musim")
    season_data = data.groupby(by=["season"],as_index=False)[["cnt"]].sum()
    st.bar_chart(season_data,x="season",y=["cnt"])
with col4:
    st.subheader("Total Pengguna Bike Sharing per Tahun")
